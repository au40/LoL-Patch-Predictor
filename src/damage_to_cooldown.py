"""
Here's my idea for this one: Damage buffs kind of show some hope. But when I look at the graphs,
I notice theres this problem that low cooldown spells getting low damage buffs are way outliared. 

This is my work in progress personal model for this issue. I'm coding this off of a different model, so there may be redundant data in here.

    delta ~ mag_base_stat + mag_damage + mag_utility + mag_other + prior_winrate + role

Usage:  python src/damage_to_cooldown.py --min-games 30
"""
...
from datadragon import champion_spell_cooldowns, change_cooldown
cooldownMap = champion_spell_cooldowns()
...
CATEGORIES = ["base_stat", "damage", "utility", "other", "damageCooldown"]
...
#this is going to define ability usage cooldown weight
#still gotta add cooldown resets from LLM extraction of data dragon descriptions to this CD weighting
def abilityUsageWeighting(change: dict) -> float:
    #change_cooldown is poorly named im sorry this is actually just the cooldown of the spell
    cd = change_cooldown(change, cooldownMap)


    if cd is None or cd == 0.0:
        #this is a passive (probably ? i cant think of any other reason i dont think)
        return None
    #model formula here 
    return min(1800.0/cd, 1800)
...
def champ_features(changes: list[dict]) -> dict:
    ...
    for ch in changes:
        ...
        #weighted damage to cooldown part of the model 
        if categorize(ch) == "damage":
            if abilityUsageWeighting(ch) is not None:
                feats["mag_damageCooldown"] += _signed_magnitude(ch) * abilityUsageWeighting(ch)
